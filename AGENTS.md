...
| 用户请求 | Skill |
|----------|-------|
| "刷 daily"、"抓论文"、"获取 arXiv" | `.github/skills/fetch-papers/SKILL.md` |
| "读论文"、"写笔记"、"解读论文" | `.github/skills/write-note/SKILL.md` |
| "批量写笔记"、"把笔记都写了" | `.github/skills/write-note-batch/SKILL.md` |
| "想 idea"、"brainstorm" | `.github/skills/gen-idea/SKILL.md` |
| "发布"、"更新站点"、"推送笔记" | `.github/skills/publish-site/SKILL.md` |

## 项目结构

- `config.yaml` — 用户配置（arXiv 类别、关注领域、评分规则、网络代理）
- `src/` — Python 脚本，可直接 `cd src && python cli.py run [DATE]`
- `notes/` — 论文笔记（mkdocs 页面）
- `logs/` — JSON 原始数据
- `paper_cache/` — 论文全文缓存
- `ideas/` — 研究 idea 文件
